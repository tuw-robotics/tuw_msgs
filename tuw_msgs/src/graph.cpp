...
std::string &Graph::frame()
{
  return this->header.frame_id;
}
const std::string &Graph::frame() const
{
  return this->header.frame_id;
}

Pose &Graph::get_origin()
{
  return static_cast<Pose &>(this->origin);
}
const Pose &Graph::get_origin() const
{
  return static_cast<const Pose &>(this->origin);
}
...
bool Graph::similar(const Graph &rhs, double epsilon_position, double epsilon_orientation) const
{
    ...
    return false;
  {
    ...
    {
      if (!static_cast<const Node &>(*it_a).similar(static_cast<const Node &>(*it_b), epsilon_position, epsilon_orientation))
        return false;
      it_a++;
      it_b++;
    }
...

  {
    ...
    {
      if (!static_cast<const Edge &>(*it_a).similar(static_cast<const Edge &>(*it_b), epsilon_position, epsilon_orientation))
        return false;
      it_a++;
      it_b++;
    }

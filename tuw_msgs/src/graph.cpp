...
{
  bool edges_eq = edges.size() == rhs.edges.size();
  for (auto it_a = edges.begin(), it_b = rhs.edges.begin();
       edges_eq & (it_a != edges.end());
       it_a++, it_b++)
  {
    const Edge &a = static_cast<const Edge &>(*it_a);
    const Edge &b = static_cast<const Edge &>(*it_b);
    if (a != b)
    {
      return false;
    }
  }
  bool nodes_eq = nodes.size() == rhs.nodes.size();
  for (auto it_a = nodes.begin(), it_b = rhs.nodes.begin();
       nodes_eq & (it_a != nodes.end());
       it_a++, it_b++)
  {
    const Node &a = static_cast<const Node &>(*it_a);
    const Node &b = static_cast<const Node &>(*it_b);
    if (a != b)
    {
      return false;
    }

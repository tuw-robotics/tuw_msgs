...
bool tuw_msgs::is_equal(const geometry_msgs::msg::Quaternion &a, const geometry_msgs::msg::Quaternion &b)
{
...
bool tuw_msgs::is_similar(const geometry_msgs::msg::Point &a, const geometry_msgs::msg::Point &b, double epsilon)
{
    return is_similar(a.x, b.x, epsilon) && is_similar(a.y, b.y, epsilon) && is_similar(a.z, b.z, epsilon);
}

bool tuw_msgs::is_similar(const geometry_msgs::msg::Quaternion &a, const geometry_msgs::msg::Quaternion &b, double epsilon)
{
    return is_similar(a.x, b.x, epsilon) && is_similar(a.y, b.y, epsilon) && is_similar(a.z, b.z, epsilon) && is_similar(a.w, b.w, epsilon);
}

bool tuw_msgs::is_similar(const geometry_msgs::msg::Pose &a, const geometry_msgs::msg::Pose &b, double epsilon_position, double epsilon_orientation)
{
    return is_similar(a.position, b.position, epsilon_position) && is_similar(a.orientation, b.orientation, epsilon_orientation);
}

void tuw_msgs::rpy_to_quaternion(double roll, double pitch, double yaw, double &qx, double &qy, double &qz, double &qw)
{
...
bool tuw_msgs::is_similar_with_cout(const std::string &a, const std::string &b)
{
    ...
    if(!is_same){
        std::cout << "a: " << "\033[0;35m" << a << "\033[0m" << std::endl;
        std::cout << "b: " << "\033[0;35m" << b << "\033[0m" << std::endl;
    }
    return is_same;
}

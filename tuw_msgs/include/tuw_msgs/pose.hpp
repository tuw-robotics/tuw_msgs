...
namespace tuw_msgs
{
  ...
  struct Pose : public geometry_msgs::msg::Pose
  {
    Pose(){
      ...
      this->orientation.x = 0, this->orientation.y = 0, this->orientation.z = 0, this->orientation.w = 1; 
    };
    ...
    Pose(const Point &p, const Quaternion &q)
    {
      ...
      this->orientation.x = q.x, this->orientation.y = q.y, this->orientation.z = q.z, this->orientation.w = q.w; 
    };
    Pose(double px, double py, double pz, double qx, double qy, double qz, double qw)
    {
      this->position.x = px, this->position.y = py, this->position.z = pz; 
      this->orientation.x = qx, this->orientation.y = qy, this->orientation.z = qz, this->orientation.w = qw; 
    };
